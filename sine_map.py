...
class SineMap:
	...
	def __init__(self, a=1.75, b=1.75):
		self.a, self.b = a, b
	...
	def f(self, xy):
		...
		return array([math.sin((math.pi * self.a * pow(x, 2)) - (self.b * y)),
				 math.sin(1 - (math.pi * self.b * x))])

	def df(self, xy, w):
		x, y = xy
		j = array([[math.cos((math.pi * self.a * pow(x, 2)) - (self.b * y))*(2*math.pi*x),
				 math.cos((math.pi * self.a * pow(x, 2)) - (self.b * y))*(-self.b)],
                   [math.sin(1 - (math.pi * self.b * x))*(-math.pi*self.b), 0]])
		return j @ w
